...
React 애플리케이션은 싱글 페이지 애플리케이션이기 때문에 라우팅을 위해 별도의 라우팅 라이브러리를 사용해야하는데  
...
React-Router에서는 이러한 동적 세그먼트를 어떻게 사용할 수 있는지 알아볼게요.
...
│  │  │  └─ticketPage/
│  │  │  │  └─ui/
│  │  │  │    └─Ticket.$ticket.tsx // 동적 세그먼트를 사용할 페이지
│  │  └─main.tsx
│  └─index.css
```
...
이렇게 `children` 속성을 기반으로 한 중첩 라우팅을 객체 기반 중첩 라우팅 이라고 해요.

React Router v6.4 이상에서는 파일 이름을 사용한 중첩 라우팅이 추가되었어요.
